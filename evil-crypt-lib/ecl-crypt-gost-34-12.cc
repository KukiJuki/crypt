		...
		uint8_t *dst_block)
{
	for (unsigned i = 0; i < 4; ++i){
		uint8_t lsn = src_block[i] & 0x0f; // least significant nibble
		uint8_t msn = (src_block[i] >> 4) & 0x0f; // most significant nibble
...
//	const uint32_t *src_a1 =
//			reinterpret_cast<const uint32_t*>(&src_block[4]);
//	const uint32_t *src_a0=
//			reinterpret_cast<const uint32_t*>(&src_block[0]);
