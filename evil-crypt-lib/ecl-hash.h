...
//будем передавать адрес некоторого блока, где необходимо разместить вектор инициализации
...
//обновление хэша -- ВАЖНО: На 8 пункте (Процедура вычисления хэш-ф) пропускаем Шаг 1.1, т.к h (малая) уже задается функцией Init
