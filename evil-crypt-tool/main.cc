// TODO: реализовать шифр из 4-го раздела госта 34.12.2018 (128 бит)
#include <iostream>
#include <iomanip>
#include <fstream>
#include <cstdio>
...
auto create_prng()
{
	...
	std::generate(std::begin(seed_data), std::end(seed_data),
			std::ref(rand_dev));
	// TODO: Возможно, усилить еще и эту последовательность
...
void generate_random_key(int length, const char *filename)
{
	...
	// 1.1. Получить доступ к датчику случайных чисел (или его максимально безопасному аналогу)
	// 1.2. Инициализировать ГПСЧ, используя данные от датчика
	auto prng = create_prng();


	// 1.3. генерируем псевдослучайную последовательность

	std::generate(std::begin(key_data), std::end(key_data), std::ref(prng));

	// TODO: Усилить псевдослучайную последовательность


	// Записываем этот массив в файл-контейнер
							...
							const char *cnt_filename)
{
	...
	for (uint32_t block_index = 0; block_index < block_count; ++block_index) {
		...
		// TODO: проверить, прочитался ли последний блок
							...
							const char *dst_filename)
{
	constexpr uint32_t BLOCK_SIZE = 8;
	uint32_t block_count;
	std::cout << "Раашифрование файла" << cnt_filename << " алгоритмом ГОСТ 34.12-64 в режиме CBC\n";
	std::ifstream key_file { key_filename, std::ios::binary };
	if (not key_file) {
		std::cerr << "Не удалось открыть файл ключа!" << std::endl;
		exit(1);
	...
	//TODO: Проверить формат и версию
